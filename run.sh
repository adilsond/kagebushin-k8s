...
#Parts copied from  https://github.com/eriksoderblom/alpine-apache-php
# Exit on non defined variables and on non zero exit codes
set -eu

echo $NODE_NAME > /htdocs/nodename.txt
chown apache:apache /htdocs/nodename.txt
sed -i "s|file_get_contents(\$filename)|\'$NODE_NAME\'|" /htdocs/index.php
#cat /dev/zero
